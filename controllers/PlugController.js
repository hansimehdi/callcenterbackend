...
module.exports = {
	addPlug: (rq, rs, nx) => {
		...
		if (error) {
			return rs.status(200).json(responseRender({ error }, serverErrors.INVALID_DATA, ''));
		}

		plugEntity.id = uuid();
		plugEntity.userId = decoder.getSubject(token);
	...
	// check the owner of the plug before fetching
	getPlug: (rq, rs, nx) => {
	...
	// check the owner of the plug before deleting
	deletePlug: (rq, rs, nx) => {
	...
	list: (rq, rs, nx) => {
		let token = rq.headers.authorization.split(' ')[1];
		dbConnect.connectToDb();
		dbConnect.getAllPlugs(decoder.getSubject(token), function(err, success) {
			dbConnect.disconnect();
			if (err) {
	...
	listAdminPlugs: (rq, rs, nx) => {
		let token = rq.headers.authorization.split(' ')[1];
		var filter = [];
		dbConnect.connectToDb();
		dbConnect.getAllUsers(decoder.getSubject(token), (err, users) => {
			if (err) {
				dbConnect.disconnect();
				return rs.status(500).json(responseRender(err, serverErrors.SERVER_ERROR, ''));
			} else {
				users.forEach(user => {
					filter.push({ userId: user.id });
				});
				dbConnect.getAdminPlugs(filter, (err, plugs) => {
					dbConnect.disconnect();
					if (err) {
						return rs.status(500).json(responseRender(err, serverErrors.SERVER_ERROR, ''));
					} else {
						return rs.status(200).json(responseRender(plugs, '', serverMessages.OK));
					}
				});
			}
		});
